...
const dashboard = () => {
  ...
  const onFinishFailedseller = (errorInfo) => {
    if (errorInfo.values["Adhar Number"]) {
      ...
      alert("Please inser Correct Adhar Card");
    }
  };

  // const onFinishbuyer = (values) => {
  //   VerifyOTP("buyer", values["Adhar Number"], values["otp"]);
  // };
  const onFinishFailedbuyer = (errorInfo) => {
    if (errorInfo.values["Adhar Number"]) {
      ...
      alert("Please inser Correct User ID");
    }
  };
  ...
  //   }
  // };
  const onFinishinspector = async (values) => {
    try {
      const response = await fetch(
        "http://localhost:8000/landInspector/login",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            username: values.username, // ✅ match backend
    ...
    <div className="bg-gray-200 max-h-screen overflow-y-auto overflow-x-hidden">
      ...
      >
        ...
        <Form
          layout="vertical"
          name="basic"
          onFinish={(values) => onFinishUserLogin("seller", values)}
                  ...
                  rules={[
                    {
                      required: true,
                      message:
                        "OTP send to your Linked Mobile Number",
                    },
                  ]}
                >
      ...
      >
        ...
        <Form
          ...
          name="basic"
          action="/lands"
          onFinish={(values) => onFinishUserLogin("buyer", values)}
